...
function ProductDetails() {
  ...
  const productLD = product ? {
    ...
    image: product.image,
    brand: { "@type": "Brand", name: "SpiceRoot" },
  ...
  return (
    <div className="min-h-screen pb-16 sm:pb-20 transition-colors duration-500">
      <SEO
        title={product.name}
        description={product.description}
        keywords={`${product.name}, SpiceRoot, ${product.category || "masala"}, Kolhapuri masala`}
        ...
        <div className="py-6 sm:py-8 min-h-[200px]">
          ...
          {activeTab === "specifications" && (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-y-4 text-xs sm:text-sm text-forest dark:text-cream/70 font-dm max-w-3xl bg-cream-light dark:bg-charcoal-light p-4 sm:p-6 md:p-8 rounded-xl sm:rounded-2xl">
              <p><strong className="text-forest dark:text-cream block mb-0.5 sm:mb-1">Brand</strong> {product.brand || "SpiceRoot"}</p>
              <p><strong className="text-forest dark:text-cream block mb-0.5 sm:mb-1">Category</strong> {product.category || "Masala Blends"}</p>
